...
# docker buildx build --tag interchainDEX --file .pi/Dockerfile --platform linux/arm64 --cache-from interchainDEX:cache --cache-to interchainDEX:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar interchainDEX
...
sudo bash -c "echo interchainDEX > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/interchainDEX.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/interchainDEX.img
